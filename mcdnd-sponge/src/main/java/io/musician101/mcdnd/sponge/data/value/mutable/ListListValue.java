...
import io.musician101.mcdnd.sponge.data.value.immutable.ImmutableListListValue;
...
public class ListListValue<E> implements ListValue<List<E>> {
    ...
    public ListListValue<E> add(int index, @Nonnull Iterable<List<E>> values) {
        int count = 0;
        for (Iterator<List<E>> iterator = values.iterator(); iterator.hasNext(); count++) {
    ...
    public ListListValue<E> filter(@Nonnull Predicate<? super List<E>> predicate) {
        ...
        list.addAll(actualValue.stream().filter(subList -> checkNotNull(predicate).test(subList)).collect(Collectors.toList()));
